...
var unzipPackage = function(archive, session, options)
{
	...
	entries.forEach(function(entry) {
		var pathParts = entry.entryName.split(path.sep);

		if ('config.json' == pathParts[ 1 ])
		{
		...
		else if ('font' == pathParts[ 1 ] && !options.updateConfigOnly)
		{
			zip.extractEntryTo(entry.entryName, options.fonts, false, options.overwrite);
		}
		else if ('css' == pathParts[ 1 ] && !options.updateConfigOnly)
		{
